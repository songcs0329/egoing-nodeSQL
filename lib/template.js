...
module.exports = {
  HTML(title, list, body, control) {
	...
	list(topics) {
		const list = topics.reduce((acc, cur) => acc += `<li><a href="/?id=${cur.id}">${sanitizeHTML(cur.title)}</a></li>`, '');
    return `<ul>${list}</ul>`;
  },
	authorSelect(authors, authorId) {
		const authorOptions = authors.reduce((acc, cur) => {
			const selected =  cur.id === authorId ? 'selected' : '';
			return acc += `<option value=${cur.id} ${selected}>${sanitizeHTML(cur.name)}</option>`
		}, '')
